...
BEGIN
		...
		SET @valorParcelasPagas = (select sum(valor) from faturas_parcelas where empresaId=new.empresaId and faturaId=new.faturaId and situacao='Paga');

    IF @valorFatura = @valorParcelasPagas THEN
			update faturas set situacao='Paga' where empresaId=new.empresaId and id=new.faturaId;
