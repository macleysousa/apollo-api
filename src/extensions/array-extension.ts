interface Array<T> {
  groupBy<TKey>(keySelector: (item: T) => TKey): GroupedItem<TKey, T>[];
  ...
  firstOrDefault(predicate?: (value: T, index: number, array: T[]) => boolean, defaultValue?: T): T;
...
Array.prototype.groupBy = function <T, TKey>(keySelector: (item: T) => TKey): GroupedItem<TKey, T>[] {
...
Array.prototype.firstOrDefault = function <T>(predicate?: (value: T, index: number, array: T[]) => boolean, defaultValue?: T): T {
  if (predicate) {
