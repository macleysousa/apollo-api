...
export class OpenpixPaymentGateway extends BasePaymentGateway {
  ...
  async createCharge(input: CreateChargeInput): Promise<CreateChargeOutput> {
    ...
    this.logger.log(`[createCharge] Iniciando cobranca OpenPix correlationID=${correlationID} amount=${input.amount}`);
    ...
    try {
      ...
      this.logger.error(`[createCharge] Falha na criacao da cobranca correlationID=${correlationID}: ${error?.message ?? error}`);
